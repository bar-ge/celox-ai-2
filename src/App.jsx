...
// Picking a language here sets fleet_lang_manual, marking it a deliberate
// choice so a later visit to the marketing site won't reset it to the region
// default. Switching country on the landing clears that flag.
function LangToggle({ lang, setLang }) {
  return (
    <div style={{ display: 'flex', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 8, overflow: 'hidden' }}>
      {['en', 'he'].map(l => (
        <button key={l} onClick={() => { setLang(l); localStorage.setItem('fleet_lang', l); localStorage.setItem('fleet_lang_manual', '1') }} style={{
